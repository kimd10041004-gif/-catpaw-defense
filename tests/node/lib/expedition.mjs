 ...
 * (3난이도 × 7맵 × 3판), 서술 범위도 "맵 사다리 × 난이도 사다리"로 못 박혀 있다. 원정은 축이 다르다.
 *
 ...
 *
 * ── U-3 · 사다리마다 파일 하나 ─────────────────────────────────────────────
 * 이 파일은 검사 본문이다. 실제 test() 등록은 `balance-sim-expedition.<사다리 id>.test.mjs` 가 `runLadderTests(id)`
 * 한 줄로 한다 — node --test 는 파일 단위로 병렬이라, 세 사다리(84초)를 한 파일에 두면 그 파일이 마지막까지 혼자 돈다.
 * 판 수는 그대로다(사다리마다 세 덱 × 시드 셋 × 4판). '뒤 사다리가 앞보다 쉽지 않다'만 선행 사다리의 룬 없음 덱을
 * 12판 더 잰다. 새 사다리를 등록하면 `balance-sim-expedition.test.mjs` 가 측정 파일이 있는지 본다.
 */
import test from 'node:test'
import assert from 'node:assert/strict'

import '../../../web/js/content/index.js'
import { getExpedition } from '../../../web/js/content/registry.js'
...
/** 덱 묶음을 잰다 — 판이 비싸므로 사다리마다 한 번만 부르고 검사들이 나눠 쓴다 */
export function measureDecks(exp, decks) {
  return decks.map((d) => {
...
/** 세 덱(최선 · 룬 없음 · 도배)을 전부 — buildTestDecks 의 순서다 */
export function measure(exp) {
  const rows = measureDecks(exp, buildTestDecks(exp))
  return { exp, best: rows[0], plain: rows[1], uniform: rows[2], rows }
}
const pct = (x) => `${Math.round(x * 100)}%`

/** 사다리 하나의 검사 일곱을 이 파일을 부른 검사 파일에 등록한다 */
export function runLadderTests(id) {
  const exp = getExpedition(id)
  if (!exp) throw new Error(`원정 '${id}' 가 등록돼 있지 않다`)
  const measured = [measure(exp)]
  ...
  // 뒤 사다리가 앞 사다리보다 쉽지 않다 — 선행 사다리의 룬 없음 덱만 한 번 더 잰다(12판). 원래는 한 파일이
  // 세 사다리를 다 재서 이웃끼리 비교했다; 파일을 나누면서 선행 사다리(`requires`)의 그 덱만 다시 잰다.
  if (exp.requires) {
    test('원정: 뒤 사다리가 앞 사다리보다 쉽지 않다', () => {
      ...
      const prevExp = getExpedition(exp.requires)
      assert.ok(prevExp, `${exp.name}: 선행 사다리 '${exp.requires}' 가 없다`)
      const prevPlain = measureDecks(prevExp, [buildTestDecks(prevExp)[1]])[0]
      const cur = measured[0]
      assert.ok(cur.plain.r.holdScore <= prevPlain.r.holdScore + 0.25,
        `${exp.name}(룬 없음 버팀 ${cur.plain.r.holdScore.toFixed(2)}) 이 ${prevExp.name}(${prevPlain.r.holdScore.toFixed(2)}) 보다 쉽다`)
    })
  }
}
