 ...
 *
 * **구간 평균으로 본다** (1~8 · 9~16 · 17~24 · 25~30). 장 단위로 못 박으면 시드 하나에 빨개졌다
...
/** 30장을 순서대로 돌면서 그때의 로스터로 잰다 — 한 번만 돌리고 나눠 쓴다 */
const rows = []
{
  let roster = [...STARTERS]
...
/** 1~8 / 9~16 / 17~24 / 25~30 구간 평균 완주율 */
const band = (from, to) => {
  const xs = rows.filter((r) => r.ch.order >= from && r.ch.order <= to)
  return xs.reduce((a, r) => a + r.clear, 0) / xs.length
}

test('시나리오: 30장이 실재하고 순서가 이어진다', () => {
  assert.equal(rows.length, 30, `챕터가 ${rows.length}장이다 — 이 검사의 구간(8·8·8·6)이 안 맞는다`)
...
test('시나리오: 뒤로 갈수록 어려워진다 (구간 평균이 내려간다)', () => {
  ...
  // U-4: 4막(유료)은 3막 위에 얹는다 — 산 사람이 3막보다 쉬운 것을 받으면 안 된다
  assert.ok(dawn <= late - 5,
    `25~30장 평균 ${dawn.toFixed(0)}% 가 17~24장 ${late.toFixed(0)}% 보다 5pp 넘게 안 낮다 — 4막이 3막보다 안 어렵다`)
})
...
test('시나리오: 후반이 벽이 되지는 않는다', () => {
  ...
  const dawn = band(25, 30)
  assert.ok(dawn >= 20, `25~30장 평균 ${dawn.toFixed(0)}% — 4막이 벽이다 (3막의 바닥 25 보다 한 눈금 아래를 허용한다)`)
})
