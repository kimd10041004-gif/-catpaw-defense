 ...
 * 값을 못 하는 코드라 뺐다.
 *
 * ── **딜러의 겹침도 재 보고 그대로 뒀다 (S)** ──────────────────────────────
 *
 * 딜러 점수는 이미 놓인 타워를 안 보므로 명당에 겹쳐 쌓인다 — 상위 12칸이 덮는 길이 단순합 220~238 대
 * 합집합 45~91 로 2.6~4.25배 겹치고, 지붕은 길의 28% 가 빈 채로 남는다. Q 에서 20장이 지을 칸을 줄였더니
 * 22% → 100% 가 된 것을 보고 "자리 고르기가 값을 흘린다"고 적었었다. **틀렸다.**
 *
 * '아직 안 덮인 길을 새로 덮는 만큼'(한계 커버리지)으로 바꾸고, 이미 k 번 덮인 지점엔 점수를 안 주는
 * 문턱 k 를 1~4 로 쓸었다 (`deck` · 보통 · 시드 7·23·11 × 3판 · 완주율):
 *
 *              지금     k=1    k=2    k=3    k=4
 *   지하실      56%     44%    33%    11%     0%
 *   다락방      44%     11%     0%    22%     0%
 *   유리 온실   22%      0%    11%    22%    11%
 *
 * 모든 문턱에서 나쁘다. 겹침은 낭비가 아니라 **길목에 화력을 모으는 것**이고, 펴면 어디서도 안 죽인다.
 * 20장은 그 맵·그 표의 우연이었다. 그래서 탐욕 배치를 그대로 둔다 — 위 아우라형과 같은 결론이다.
 * 다시 시도할 사람을 위해 적어 둔다: 이 방향은 이미 재 봤다.
 */
