 ...
 *
 * **어디에 곱하나**: `game.js` 의 `applyDamage` 가 **방어력을 빼기 전에** 곱한다 (L-2 에서 옮겼다).
 * 원래는 뺀 뒤였다 — "앞에 곱하면 ×1.5 가 적마다 다른 값이 돼 화면에서 안 읽힌다"는 이유. 그런데 그 순서는
 * 저피해 속사(치즈냥 1발 12)가 장갑 12 앞에서 바닥 1 로 눌린 뒤 ×1.5 를 받아 +0.5/발이 되게 했다.
 * 상성이 장갑을 뚫는 값이어야 사람이 느낀다고 보고 앞으로 옮겼다.
 *
