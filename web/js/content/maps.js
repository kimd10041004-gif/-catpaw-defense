...
registerMap({
  ...
  blocked: [[0, 0], [8, 13]],
  tier: 3,
  // 값은 그대로 뒀다 (M). 여기도 `deck` 봇이 목숨을 하나도 안 잃는다(남은 목숨 1.00) — 창고와 같은 공짜인데,
  // **반응이 경사가 아니라 절벽이라 쓸 값이 없다.** 골드로 초반을 사면서 쓸어 본 것(시드 7·23·11 × 3판):
  //   1.20 ~ 1.39 / 골드 380  → 남은 목숨이 전부 **1.00** (완주율 100%). 열 칸을 재도 한 칸도 안 내려온다
  //   1.40                   → 완주율이 67~89% 로 떨어진다. 티어 1~3 의 100% 는 지킨다 — 첫 맵들이 막히면 안 된다
  //   1.45                   → 완주율은 100% 로 돌아오지만 남은 목숨이 0.13~0.20 이라 **창고(0.33)보다 아래**다
  // 1.00 과 0.20 사이에 값이 없다. 억지로 올리면 티어 3 이 티어 4 보다 어려워져 사다리가 또 뒤집힌다.
  hpMul: 1.10,
  startGold: 260,
  startLives: 15,
  waveSet: 'rooftop30',
