...
registerMap({
  ...
  /* 2·6·10행을 통째로 막는다 (Q). **이 맵의 손잡이는 체력도 마릿수도 아니고 여기다.**
   *
   * M 은 `hpMul` 로, Q 는 웨이브 표로 지붕을 고치려다 둘 다 실패했다. 재 본 것:
   *   hpMul  1.20~1.39 → 남은 목숨 전부 1.00 · 1.40 → 완주율이 깨짐 · 1.45 → 0.13~0.20(창고보다 아래)
   *   웨이브 두더지 +1,680마리 · 불개미 +1,344마리 · 박쥐 ×4배 → **전부 1.00**. 박쥐왕 60마리 → 전멸
   * 아예 안 듣다가 한 번에 전멸한다. 절벽이 아니라 벽이었다.
   *
   * 원인은 길 모양이다. 지붕은 길이 46칸에 꺾임 13번인 지그재그라 **타워 한 채가 여러 줄을 동시에 본다**
   * (창고는 32칸·9꺾임). 봇이 49채를 짓고, 늘린 적이 전부 사거리 안에서 죽는다. 그래서 손잡이는
   * 적의 양이 아니라 **타워가 설 자리**다. 빈 줄(2·4·6·8·10행)을 막아 가며 쓸었다(시드 7·23·11 × 3판):
   *   지을 칸 79(지금) → 1.00 · 56 → 0.73 · **55(2·6·10 통째) → 0.47** · 55(4·6·8) → 0.20(완주율 89%)
   *   53 → 0.13(완주율 78%)
   * 2·6·10 을 골랐다. 시드를 통째로 바꿔도(3·41·59) 0.47 로 같다 — 칼날 위가 아니다.
   * 건설 가능 타일은 55 로 `content.test` 의 하한 40 을 지킨다. 막힌 칸은 `props` 로 그려진다.
   *
   * **7열은 빼 둔다.** (7,2)·(7,6)·(7,10) 은 길 타일이라 `isBuildable` 이 이미 막는데,
   * `blocked` 에 넣으면 `_drawBlocked` 가 길 위에 상자를 그린다(찍어 보고 알았다). 값은 그대로 55 다.
   *
   * 지붕 맵을 쓰는 6장·15장·서릿길은 안 움직였고, 7장·20장과 잿불 길·천둥 고개의 지붕 칸은
   * 같이 움직여서 각자 자리에서 보정했다(주석은 그쪽에 있다). */
  blocked: [
    [0, 0], [8, 13],
    [0, 2], [1, 2], [2, 2], [3, 2], [4, 2], [5, 2], [6, 2], [8, 2],
    [0, 6], [1, 6], [2, 6], [3, 6], [4, 6], [5, 6], [6, 6], [8, 6],
    [0, 10], [1, 10], [2, 10], [3, 10], [4, 10], [5, 10], [6, 10], [8, 10],
  ],
  tier: 3,
  hpMul: 1.10,
  startGold: 260,
...
registerMap({
  ...
  blocked: [[0, 13], [8, 0]],
  tier: 6,
  // 0.72 / 420 이었다 (L-1). `deck` 봇 0% — 지하실과 같은 벽. 여기서는 시작 골드가 **살아 있는** 손잡이다
  // (nightmare20 은 1웨이브 총 체력 906 으로 앞이 무겁다): 체력 ×0.86 만으로는 0~22% 인데 골드 480 을 같이 주면
  // 44~56% 다. 값(시드 셋 × 3판): 0.62/480 → 56% · 0.60/480 → 44% · 0.58/420 → 22%. 지하실(67%)과 온실(11%)
  // 사이 계단이 되게 0.60/480 을 골랐다. 바닥 봇 첫 실점 20웨이브 그대로.
  hpMul: 0.60,
  startGold: 480,
  startLives: 15,
  waveSet: 'nightmare20',
...
registerMap({
  ...
  tier: 4,
  // 1.05 / 300 이었다 (M). `deck` 봇 완주율이 티어 1~4 에서 **전부 100%** 라 넷이 한 덩어리로 보였는데,
  // 남은 목숨으로 보니 창고는 아홉 판 전부 **한 대도 안 맞고** 끝나 있었다(1.00) — 티어 4 가 공짜였다.
  // L 에서 "창고는 손잡이가 없다(1.20 까지 올려도 100%)"고 적은 것은 **완주율로만 쟀기 때문**이다.
  // 목숨으로 다시 재니 움직인다 (시드 7·23·11 × 3판 · 괄호는 치즈냥 첫 실점/중앙):
  //   1.05/300 → 1.00   1.20/300 → 0.56   1.20/360 → 0.56   1.20/400 → 0.33   1.15/400 → 0.78
  //   1.20/440 → 0.28   1.20/480 → 0.56   1.20/520 → 0.31   **1.15/440 → 0.33**
  //
  // 체력만 올리면 **약한 봇의 바닥이 무너진다.** 1.20/400 은 잘 두는 봇을 0.33 으로 내리지만 바닥선 둘을
  // 같이 깼다 — 치즈냥 도달 점수가 29 → 14 로 떨어져 지하실보다 나빠지고(사다리 뒤집힘),
  // 길냥이(어려움)에서 smart 봇 첫 실점이 4웨이브다. **후보를 하나씩 재면 바닥을 하나씩 발견하게 된다** —
  // 그래서 그물 아홉을 한 번에 대는 표로 골랐다(완주율·목숨·치즈 도달/첫실점/중앙 · 세 난이도의 첫실점/중앙/역전).
  //
  // 온실에서 배운 대로 **시작 골드로 초반을 사고 체력으로 뒤를 갚는다** — 골드는 잘 두는 봇에겐 이미 남는
  // 자원이라(온실 주석의 ×0.3 측정) 난이도를 안 되돌리고, 약한 봇의 첫 배치만 산다. 통과한 넷 중 1.15/440 은
  // 여유가 가장 넓다: 치즈냥 첫 실점 15(바닥 5) · 도달 29.0(바꾸기 전과 같다) · 길냥이 smart 첫 실점 15(바닥 5) ·
  // 중앙 30(바닥 10). 1.20/480·1.20/520 은 길냥이 중앙이 딱 10 이라 여유가 0 이고, 1.15/400·1.10/400 은
  // 목숨 0.78 로 '공짜' 선(0.85)에 붙는다. 이 맵을 쓰는 8·9·16·21장은 실효값이 K 때와 같게 보정했다.
  hpMul: 1.15,
  startGold: 440,
  startLives: 18,
  waveSet: 'warehouse30',
...
registerMap({
  ...
  tier: 5,
  // 1.20 이었다 (L-1). 잘 두는 봇(`deck`) 완주율이 티어 1~4 **100%** 에서 여기 **0%** 로 뚝 떨어졌다 —
  // 사다리가 계단이 아니라 벽이었다. 0.05 씩 내리며 잰 값(시드 7·23·11 × 3판): 1.00 → 44% · 0.95 → 67%(셋 다 67) ·
  // 0.90 → 67%. 시작 골드는 손잡이가 아니었다(×1.3 에도 0%). 바닥 봇(치즈냥) 첫 실점 7웨이브 · 길냥이 첫 실점 20 유지.
  hpMul: 0.95,
  startGold: 340,
  startLives: 15,
  waveSet: 'basement30',
...
/* ── 원정 전용 맵 두 개 — 격자가 다르다 ─────────────────────────────────────
 *
 * 자유 맵은 처음 여섯이 전부 9×14 다(일곱째 유리 온실은 9×13). 9×14 를 하나 더 넣는 것은 "길 모양 하나 더"일 뿐이라,
...
registerMap({
  ...
  // L — 사다리를 계단으로 만들 때 시드 셋(7·23·11)으로 다시 재니 1.80 은 **0%** 였다(11% 는 시드 하나 값).
  //   절벽이 가파르다: 1.70 → 56% · 1.60 → 78% · 1.80+골드 720 → 44%. 1.74 · 1.76 · 1.78 이 전부 22% 라 가운데를 골랐다.
  //   치즈냥 첫 실점은 그대로 8웨이브. 티어 6 다락방(44%) 아래고 절벽 검사(50%p)·뒤집힘 검사(25%p) 안이다.
  hpMul: 1.76,
  startGold: 660,
  startLives: 16,
