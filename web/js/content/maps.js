...
registerMap({
  ...
  tier: 7,
  // hpMul 1.00 · startGold 440 이었다. 티어 7 인데 `deck` 봇 완주율이 **100%** 로 티어 5(지하실)·
  // 6(다락방)의 0% 보다 쉬웠다 — 사다리가 뒤집혀 있었다. J-5 가 못 잡은 이유는 검사가 약한
  // 봇(cheese)의 **도달 점수**를 봤기 때문이다(0% 와 100% 가 19.0 과 21.0 으로 눌린다).
  //
  // 손잡이를 다 재 봤는데 `deck` 봇에게 **체력 말고는 아무것도 안 통했다**:
  //   startGold ×0.3 → 100% · startLives ×0.25(4목숨) → 100% · armorAdd 6 → 100% ·
  //   speedMul 1.3 → 100% · 13~20웨이브 마릿수 +40% → 100%
  // 봇은 돈을 다 안 쓰고 목숨도 안 깎이며, 화력이 남아 마릿수는 안 아프다.
  //
  // 그런데 체력만 올리면 **바닥 봇(치즈냥)의 초반이 무너진다** — 어려움에서 hpMul 1.25 만 돼도
  // 첫 실점이 2웨이브다('짓기도 전에 뚫린다' 검사). 유리 온실은 앞이 가볍기 때문이다:
  // 1웨이브 330 (다락방 906) · 5웨이브 2030 (다락방 3564).
  // 그래서 **둘을 같이 움직인다**: 시작 골드로 초반을 사고, 체력으로 뒤를 갚는다.
  // 골드는 잘 두는 봇에겐 이미 남는 자원이라(위 ×0.3 측정) 난이도를 안 되돌린다.
  //   ×1.80 · 660골드 → `deck` 완주 11%(시드 여섯) · 아깽이·집냥이·길냥이 모두 첫 실점 8웨이브
  hpMul: 1.80,
  startGold: 660,
  startLives: 16,
