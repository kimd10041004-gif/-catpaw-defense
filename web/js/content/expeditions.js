 ...
 *
 *   **Q — 새는 것은 `hpMul` 만이 아니다. 기하도 샌다.** 지붕의 `blocked` 에 2·6·10행을 더해(§5) 지을 수 있는
 *   칸을 79 → 55 로 줄이자 지붕 칸 둘이 **쉬워졌다** — 선택지를 뺐더니 봇이 더 잘 뒀다. `맵.hpMul` 은 1.10
 *   그대로라 위 곱셈으로는 이 누수가 안 보인다. 칸의 `hpMul` 로 되돌렸다:
 *     · 잿불 길 지붕 칸 1.45 → **1.65** (룬 없음 완주 17% · 버팀 5.07 — 고치기 전 값 그대로)
 *     · 천둥 고개 지붕 칸 1.38 → **1.56** (25% · 4.67 — 고치기 전 17% · 4.78)
 *   천둥 쪽은 안 고쳤으면 *'뒤 사다리가 앞 사다리보다 쉽지 않다'* 가 빨갰다(버팀 5.43 > 서릿길 5.08 + 0.25).
 *   서릿길은 지붕을 안 써서 한 칸도 안 움직였다.
 *
...
registerExpedition({
  ...
  stages: [
    ...
    {
      // Q — 지붕 맵 보정. `blocked` 를 바꾸자 이 칸이 쉬워져 룬 없는 덱 완주율이 17% → 50% 로 튀었다.
      // 쓸이(시드 7·23·11 × 4판): 1.45 → 50%/5.55 · 1.55 → 42%/5.40 · **1.65 → 17%/5.07** · 1.75 → 8%/4.94
      // 1.65 가 고치기 전 값(17% · 버팀 5.07)으로 정확히 돌아온다.
      mapId: 'rooftop', waveSet: 'rooftop30', waveLimit: 10, element: 'ice', hpMul: 1.65, boss: 'glowpigeon',
...
registerExpedition({
  ...
  stages: [
    ...
    {
      /* Q — 지붕 맵 보정. 잿불 길과 같은 이유인데 여기가 더 아팠다: 룬 없는 덱 버팀이 4.78 → 5.43 으로
       * 올라 **'뒤 사다리가 앞 사다리보다 쉽지 않다'(서릿길 5.08 + 0.25 = 5.33)가 빨개졌다.**
       * 쓸이(시드 7·23·11 × 4판 · 룬 없음): 1.38 → 42%/5.43 · 1.48 → 58%/5.52 · 1.50 → 33%/5.06 ·
       *   1.52 → 42%/5.12 · 1.54 → 33%/4.99 · **1.56 → 25%/4.67** · 1.60 → 8%/4.60 · 1.58 → 0%/4.27
       * 1.56 이 고치기 전(17% · 4.78)에 가장 가깝고 사다리 순서에 0.66 여유가 남는다. */
      mapId: 'rooftop', waveSet: 'rooftop30', waveLimit: 10, element: 'fire', hpMul: 1.56, boss: 'roachqueen',
