...
registerObjective('makeCombo', {
  ...
  label: (spec) => tr('\'{v}\' 조합 만들기', { v: (getCombo(spec.comboId) || { name: spec.comboId }).name }),
  check: (s, spec) => (s.combosMade || []).includes(spec.comboId),
  requires: ['comboId'],
})

registerObjective('killAtLeast', {
  label: (spec) => tr('해충 {n}마리 이상 처치', { n: spec.n }),
