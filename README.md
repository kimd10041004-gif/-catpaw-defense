...
거의 제자리 · 대기 · 자는 중), 해충은 3장(걷기 A · 걷기 B · 멈춤). 새 보스 셋은 아직 그림이 없어 벡터로 나온다.
...
- **속성 원정** — 맵 목록 맨 위. **사다리 둘**(잿불 길 → 서릿길). 여섯 칸을 **목숨 하나로** 잇는다. **덱은 네 마리**뿐이고,
  칸마다 해충의 지배 속성이 다르다(흙 → 번개 → 얼음 → 불 → 어둠 → 빛). 상성이 걸리는 유일한 모드다.
...
**한 속성으로 도배하는 것은 함정이다.** 원정 여섯 칸이 여섯 속성을 한 칸씩 쓰므로,
같은 속성으로 넷을 채우면 그 속성이 약한 칸에서 네 마리가 전부 불리해진다. 봇으로 재면
최선 룬 덱 완주 50~67% · 룬 없음 17~33% · 도배 0% 다(`docs/확장가이드.md` §21 에 표가 있다).
