...
> 이 주소에는 GitHub 계정 이름이 그대로 들어 있다. **도메인으로 바꾸려고 준비해 뒀다 (R)** —
> 코드 쪽은 `web/js/build.js` 의 `FULL_APP_URL` **한 줄**이면 끝나고, 계정 이름이 다시 새면
> `site.test` 가 잡는다. 남은 것은 도메인을 사는 것뿐이다
> ([절차는 `docs/확장가이드.md` §17](docs/확장가이드.md)).

| | |
|---|---|
| **안드로이드** | APK 한 번 탭 ([`docs/실기기설치.md`](docs/실기기설치.md)) — 사이트가 APK 를 직접 준다. GitHub 을 안 거친다 |
...
# → dist/catpaw-defense.html  (완전 독립 — 아트를 전부 인라인해서 10MB 대다)
```

브라우저로 그냥 열면 끝이다. ES 모듈을 각자 함수 스코프로 감싸 합쳤기 때문에
`file://`에서도 CORS에 막히지 않는다. 폰에 파일을 넣어서 열어도 되고, 어디든 올려도 된다.
정확한 크기와 모듈 수는 `bundle.mjs` 가 돌 때마다 마지막 줄에 찍는다 — 여기 손으로 안 적는다
(적어 뒀더니 220KB · 모듈 25개인 채로 굳어 50배 틀린 값이 남아 있었다).
...
- **시나리오 30장 (무료 1~2막 18장 + 유료 3막 6장 · 4막 6장)** — 컷신으로 이어지는 캠페인. 장마다 주 목표 1개 + 부 목표 2개가
  있고 별을 최대 3개까지 받는다. **장마다 난이도가 다르다** — 챕터가 `rules`(적 체력 배수)를 받아서
  뒤로 갈수록 압박이 커진다. 잘 두는 봇(`deck` · 시드 셋 × 3판) 구간 평균 완주율 1~8장 **89%** · 9~16장 **78%** · 17~24장 **40%** · 25~30장(유료 4막) **33%** (`scenario-curve.test` 와 같은 방법 — Q 에서 다시 잼, 4막은 U-4). **봇이 한 번도 못 깨는 장은 없다** — 검사가 장마다 바닥을 깐다. 12·18·24장이 22% 로 그 막에서 가장 어렵다. 고양이 세 마리(샴냥·검은냥·뚱냥)가 2·4·6장 보상으로
...
- **속성 원정** — 맵 목록 맨 위. **사다리 넷**(잿불 길 → 서릿길 → 천둥 고개 → 새벽 능선). 여섯 칸을 **목숨 하나로** 잇는다.
  **덱은 네 마리**뿐이고, 칸마다 **잡몹의** 지배 속성이 다르다(흙 → 번개 → 얼음 → 불 → 어둠 → 빛).
  상성이 걸리는 유일한 모드다. **넷 중 셋(잿불 길 · 천둥 고개 · 새벽 능선)은 칸마다 보스도 고르는데, 그 보스는 지배 속성에 안 덮여
...
(사다리 넷의 표는 `docs/속성과카드.md` "지금 밸런스").
...
| 필살기 | 효과 | 속성 | 마나 · 쿨다운 |
|---|---|---|---|
| 🍥 츄르 폭격 | 화면의 모든 적에게 큰 피해 (웨이브에 따라 위력 상승) | **무속성** | 60 · 20초 |
| 💤 자장가 | 모든 적 **75% 둔화 5초** | — | 35 · 18초 |
| 🥛 우유 홍수 | 지상 적 전체 피해 + 둔화 (공중은 안 맞는다) | ❄ 얼음 | 50 · 22초 |
| ✨ 황금 발바닥 | 모든 고양이 **공격 속도 2.2배, 10초** | — | 45 · 25초 |
| 😾 하악질 | 화면의 모든 적 **장갑 −4, 6초** — 장갑을 정면으로 맞는 광역기 넷의 약점을 메운다 | — | 40 · 20초 |
| 🌀 헤어볼 | **체력이 가장 높은 적 하나**에 큰 피해 — 보스용 한 방 | ☾ 어둠 | 55 · 24초 |

**속성 — 피해를 주는 것만 가진다.** 여섯 중 피해기는 셋(츄르·우유·헤어볼)이고, 그중 **둘**에만 속성이 있다.
원정에서만 상성이 걸린다(유리 ×1.5 · 불리 ×0.7). 둔화·버프·장갑 벗기기에는 곱할 피해가 없어 속성이 없다.
**츄르 폭격은 일부러 무속성으로 남겼다** — 넷이 다 속성이면 로드아웃 고르기가 "이번 사다리 색 맞추기"
한 줄로 줄어든다. 무속성 한 방이 있어야 *맞춰서 크게* 와 *어느 색이든 안 흔들리게* 가 서로 다른 선택이 된다.
...
`deck` 봇 · 보통 · 시드 셋 기준 **0.80 · 0.58 · 0.47 · 0.33 · 0.09 · 0.07 · 0.00**. 완주율로는 티어 1~4 가 한 덩어리인데
목숨으로 보면 사다리가 보인다. 시뮬레이터 표와 `--json` 에 '남은 목숨' 칸이 있다.
...
**Q 에서 지붕을 고쳤다 — 손잡이가 체력도 마릿수도 아니었다.** M 은 `hpMul` 로 실패했고, 다음으로 잡은 웨이브 표도
실패했다: `ROOFTOP_30` 뒷줄에 두더지 1,680마리를 더 넣어도 **남은 목숨이 1.00 에서 안 움직인다**(박쥐왕 60마리를
한 줄에 넣으면 그제야 전멸한다 — 절벽이 아니라 벽이다). 원인은 길 모양이었다. 지붕은 길이 46칸에 꺾임 13번인
지그재그라 타워 한 채가 여러 줄을 동시에 보고, 봇이 49채를 짓는다(창고는 32칸·9꺾임·46채). 그래서 손잡이는
적의 양이 아니라 **타워가 설 자리**다 — 2·6·10행을 막아 지을 수 있는 칸을 79 → **55** 로 줄이자 남은 목숨이
1.00 → **0.47**, 완주율은 100% 그대로다(시드를 통째로 바꿔도 0.47). 새 검사가 *"완주율 100% 인 맵 중 첫 맵보다
후한 맵은 없다"* 를 못 박는다 — M 의 검사는 구간의 *마지막* 칸만 봐서 가운데가 솟은 것을 못 잡았다.
지붕을 쓰는 7·20장과 잿불 길·천둥 고개의 지붕 칸 넷은 전 값으로 보정했다. 쓸이표는 `docs/확장가이드.md` §5.

**T 에서 부엌을 고쳤다.** 부엌(t2)은 0.40 으로 지붕(0.47)보다 낮게 꺼져 있었고, 원인은 `kitchen30` 30웨이브 한 줄이었다 —
마왕·바퀴 여왕 둘·쥐왕이 5초 안에 겹쳐 들어와 덱 봇의 목숨을 거기서만 깎았다(`bleedWaves` 0). 보스 셋의 간격만
벌려(5→9 · 10→16) **0.58**, 완주율 100% 그대로다(시드 셋을 통째로 두 번 바꿔도 0.58·0.60). 바퀴 수를 줄이면 0.40
그대로고(원인이 아니다), 바퀴를 늦추면 0.80 으로 골목길과 같아진다. 11~30웨이브는 자유 모드만 쓰므로(3장과 원정 두 칸은
10웨이브까지) 시나리오·원정 보정은 없다. 사다리는 이제 **0.80 → 0.58 → 0.47 → 0.33** 으로 단조다. 쓸이표는 §5.
...
| 캣닢으로 | 실제 결제로 |
|---|---|
| 이어하기 · 골드 러시 · 필살기 충전 · 목숨 보충 · 펫 · **훈련**(고양이당 3단계) · **필살기 성장**(6종 × 세기·쿨다운 3단계) · 스킨 3종 | 캣닢 충전 2종 · 프리미엄 팩 · **스타터 팩 · 시나리오 3막 · 4막 · 도전 팩 2 · 스킨 팩 2개** |
...
지금은 실제 청구가 **일어나지 않는다.** 웹은 "데모 결제 (실제 청구 없음)"이고, 안드로이드는 Play Console 에 상품을
등록하기 전까지 "Google Play 결제 (미설정)"으로 표시되며 결제 시도는 정직하게 실패한다. 브리지 자체(Play Billing Library 9)는
구현돼 있다 — 상품이 등록되면 켜진다(`docs/결제연동.md`).
...
web/art/           프레임 아트 스트립 50장 (고양이 5프레임 · 해충 3프레임, 칸은 169~396px
