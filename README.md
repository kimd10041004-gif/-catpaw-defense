...
거의 제자리 · 대기 · 자는 중), 해충 10종은 3장(걷기 A · 걷기 B · 멈춤).
...
- 맵 4종 — 골목길 → 부엌 → 지붕 → **악몽의 다락방**(20웨이브 내내 보스가 쏟아진다). 클리어하면 다음 맵이 열린다
...
맵 클리어 20. **한 판을 다 깨면 52개**가 모이고, 이어하기 한 번이 50이다.

| 캣닢으로 | 실제 결제로 |
|---|---|
| 이어하기(목숨 +10) · 골드 러시 · 필살기 충전 · 목숨 보충 | 캣닢 충전 2종 · 프리미엄 팩(시작 골드 +200, 캣닢 2배) |

**유료로만 얻는 타워·맵·보스는 없다.** 결제 없이 전 맵을 클리어할 수 있게 설계했고,
그 조건은 `economy.test.mjs`가 자동으로 검사한다.
